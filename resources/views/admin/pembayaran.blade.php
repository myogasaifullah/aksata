                            ...
                            <tbody>
                                ...
                                <tr class="payment-row" data-id="{{ $payment->id }}">
                                    ...
                                    <td class="py-2 px-4 border-b">
                                        <button type="button" class="btn btn-link text-dark px-3 mb-0 editPaymentBtn" 
                                                data-id="{{ $payment->id }}" data-bs-toggle="modal" data-bs-target="#editPaymentModal">
                                            <i class="material-icons text-sm me-2">edit</i>Edit
                                        </button>
                                        <button type="button" class="btn btn-link text-danger text-gradient px-3 mb-0 deletePaymentBtn" 
                                                data-id="{{ $payment->id }}" 
        ...
        <div class="modal-dialog">
            <form id="editPaymentForm" method="POST" enctype="multipart/form-data" class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        <label for="editMethod" class="form-label">Bank</label>
    ...
    <div class="modal fade" id="deletePaymentModal" tabindex="-1" aria-labelledby="deletePaymentModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <p>Apakah Anda yakin ingin menghapus pembayaran <strong id="deletePaymentName"></strong>?</p>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Batal</button>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // Tombol Edit - tetap menggunakan AJAX untuk load data
    ...
    // Tombol Delete - menggunakan form submit biasa
    document.querySelectorAll('.deletePaymentBtn').forEach(button => {
        button.addEventListener('click', function () {
            const paymentId = this.getAttribute('data-id');
            const paymentMethod = this.getAttribute('data-method');
            
            // Set nama pembayaran yang akan dihapus
            document.getElementById('deletePaymentName').textContent = paymentMethod;
            
            // Set action form untuk delete
            document.getElementById('deletePaymentForm').action = `/pembayaran/${paymentId}`;

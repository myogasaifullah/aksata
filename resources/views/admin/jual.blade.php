                            ...
                            <div class="table-responsive p-0">
                                <table class="table align-items-center mb-0">
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            <td><p class="text-xs font-weight-bold mb-0">{{ $jual->game->nama ?? 'N/A' }}</p></td>
                                            <td><p class="text-xs mb-0">{{ $jual->jumlah }}</p></td>
                                            <td class="align-middle text-center">
                                                <span class="text-secondary text-xs font-weight-bold">Rp {{ number_format($jual->harga, 0, ',', '.') }}</span>
                                            </td>
                                            <td class="align-middle text-center">
                                                ...
                                                <button type="button" class="btn btn-link text-danger text-gradient px-3 mb-0 delete-jual-btn"
                                                    data-bs-toggle="modal" data-bs-target="#deleteJualModal"
                                                    data-id="{{ $jual->id }}"
                                                    data-game="{{ $jual->game->nama ?? 'N/A' }}">
                ...
                <form method="POST" action="{{ route('admin.jual.store') }}">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" name="jumlah" id="jumlah" class="form-control" placeholder="Contoh: 100 Diamond" required>
                        </div>
                        <div class="mb-3">
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <form id="deleteJualForm" method="POST" action="">
                    ...
                    <div class="modal-body">
                        <p>Apakah Anda yakin ingin menghapus data jual untuk game "<span id="deleteJualGame"></span>"?</p>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Batal</button>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.querySelectorAll('.edit-jual-btn').forEach(button => {
        button.addEventListener('click', function() {
            ...
            // Update action form
            editJualForm.action = `{{ route('admin.jual.index') }}/${id}`;
    ...
    document.querySelectorAll('.delete-jual-btn').forEach(button => {
        button.addEventListener('click', function() {
            ...
            deleteJualForm.action = `{{ route('admin.jual.index') }}/${id}`;
            document.getElementById('deleteJualGame').textContent = gameName;
        });
    });
});
</script>
